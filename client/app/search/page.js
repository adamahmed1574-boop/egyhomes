...
function SearchContent() {
  ...
  const fetchProperties = async () => {
    ...
    // Use the NAME of the governorate to find Cities
    const govName = filters.governorate;
  ...
  // Helper to find Gov Name from ID or just return value if it is a name
  const getCities = (govVal) => {
      // govVal could be "1" (ID) or "Cairo" (Name). 
      // If "1", convert to "Cairo". If "Cairo", use as is.
      const name = GOVERNORATES[govVal] || govVal;
      return CITIES[name] || [];
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {!showFavorites && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
                ...
                <div className="space-y-1">
                    ...
                    <select className="w-full p-3 rounded-xl border bg-gray-50 dark:bg-zinc-800 dark:border-zinc-700 dark:text-white" value={filters.governorate} onChange={e => setFilters({...filters, governorate: GOVERNORATES[e.target.value] || e.target.value, city: 'All'})}>
                ...
                <div className="space-y-1">
                    ...
                    <option value="All">All</option>
                    {getCities(filters.governorate).map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                </div>
                ...
                <div className="space-y-1">
                    ...
                    <option value="All">Any</option><option value="Apartment">Apartment</option><option value="Villa">Villa</option><option value="Duplex">Duplex</option><option value="Commercial">Commercial</option><option value="Chalet">Chalet</option>
                    </select>
                </div>
        ...
        <div className="mb-6">
            <button onClick={() => { setShowFavorites(!showFavorites); }} className={`text-sm font-bold flex items-center gap-2 ${showFavorites ? 'text-emerald-500' : 'text-gray-500'}`}>
                {showFavorites ? '← Back to All Properties' : 
                 <span className="flex items-center gap-2"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.7l-1.1-1.1a5.5 5.5 0 0 0-7.8 7.8l1.1 1.1L12 21.2l7.8-7.8 1.1-1.1a5.5 5.5 0 0 0 0-7.8z"/></svg> View My Favorites Only</span>
                }
            </button>
        </div>
