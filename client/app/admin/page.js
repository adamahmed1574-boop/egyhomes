      ...
      <div className="max-w-5xl mx-auto bg-white p-8 rounded-3xl shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
            ...
            <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4 bg-blue-50 p-4 rounded-xl border border-blue-100">
                ...
                <div>
                    <label className="text-xs font-bold text-gray-500 ml-1">Plans (Months)</label>
            ...
            {properties.map(p => (
                <div key={p._id} className="flex flex-col md:flex-row justify-between items-center border p-4 rounded-xl hover:shadow-md transition bg-gray-50">
                    ...
                    <div className="flex gap-2 w-full md:w-auto">
                        <button onClick={() => startEdit(p)} className="bg-blue-100 text-blue-600 px-4 py-2 rounded-lg font-bold flex-1">Edit</button>
                        <button onClick={() => deleteProperty(p._id)} className="bg-red-100 text-red-600 px-4 py-2 rounded-lg font-bold flex-1">Delete</button>
                    </div>
                </div>
            ))}
        </div>
        ...
        <div className="mt-16 pt-8 border-t-2 border-gray-100">
             ...
             <form onSubmit={addPartner} className="flex gap-4 mb-6">
                 ...
                 <button className="bg-purple-600 text-white px-6 rounded-xl font-bold">Add</button>
             </form>
             <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                 {partners.map(p => (
                     <div key={p._id} className="border p-4 rounded-xl flex justify-between items-center bg-white">
                         <span className="font-bold">{p.name}</span>
                         <button onClick={() => deletePartner(p._id)} className="text-red-500 font-bold">✕</button>
                     </div>
                 ))}
             </div>
        </div>
