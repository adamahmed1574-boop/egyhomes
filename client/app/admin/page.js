...
export default function AdminPanel() {
  ...
  // FIX: Clean the URL to ensure no double slashes
  const rawUrl = process.env.NEXT_PUBLIC_API_URL || '';
  const API_URL = rawUrl.endsWith('/') ? rawUrl.slice(0, -1) : rawUrl;

  const checkLogin = () => {
    // Basic check, real check happens on server
    if (secret) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Logic: Convert Gov ID "1" to Name "Cairo"
    const govName = GOVERNORATES[form.governorate] || form.governorate;
    let cityName = form.city;
    // Logic: Default City
    if (!cityName && CITIES[form.governorate]) cityName = CITIES[form.governorate][0];
    ...
    const payload = { 
        ...
        secret: secret // CRITICAL: Send Password to Server
    };
    ...
    try {
        ...
        if(res.ok) {
            ...
            alert("Failed: " + (data.error || "Check Password in Vercel Settings"));
        }
    } catch (err) {
  ...
  const deleteProperty = async (id) => {
    if(confirm("Delete?")) {
        const res = await fetch(`${API_URL}/api/properties/${id}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ secret }) });
        if (res.ok) fetchData();
        else alert("Delete Failed. Check Password.");
    }
  };

  // ... (Keep startEdit, addPartner, deletePartner logic same as before) ...
  const startEdit = (p) => {
      setEditingId(p._id);
  ...
  const addPartner = async (e) => { e.preventDefault(); await fetch(`${API_URL}/api/partners`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...partnerForm, secret }) }); setPartnerForm({ name: '', logo: '' }); fetchData(); };
  const deletePartner = async (id) => { if(confirm("Delete?")) { await fetch(`${API_URL}/api/partners/${id}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ secret }) }); fetchData(); } };

  if (!isAuthenticated) return <div className="h-screen flex items-center justify-center bg-gray-100"><div className="p-8 bg-white rounded shadow-md"><h2 className="text-2xl font-bold mb-4 text-blue-900">Admin Login</h2><input type="password" onChange={e=>setSecret(e.target.value)} className="p-3 border rounded w-full mb-4" placeholder="Enter Secret Code"/> <button onClick={checkLogin} className="bg-blue-900 text-white w-full p-3 rounded font-bold">Login</button></div></div>;
  
  const currentCities = CITIES[form.governorate] || [];
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans text-slate-800">
      <div className="max-w-5xl mx-auto bg-white p-8 rounded-3xl shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
            ...
            <div className="space-y-1"><label className="text-xs font-bold text-gray-500 ml-1">Governorate</label><select value={form.governorate} onChange={e=>setForm({...form, governorate: e.target.value, city: ''})} className="border p-3 rounded-xl w-full">{Object.entries(GOVERNORATES).map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select></div>
            <div className="space-y-1"><label className="text-xs font-bold text-gray-500 ml-1">City</label><select value={form.city} onChange={e=>setForm({...form, city: e.target.value})} className="border p-3 rounded-xl w-full"><option value="" disabled>Select City</option>{currentCities.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
            ...
            <div className="grid grid-cols-3 gap-2"><input type="number" placeholder="Beds" value={form.bedrooms} onChange={e=>setForm({...form, bedrooms: e.target.value})} className="border p-3 rounded-xl" /><input type="number" placeholder="Baths" value={form.bathrooms} onChange={e=>setForm({...form, bathrooms: e.target.value})} className="border p-3 rounded-xl" /><input type="number" placeholder="Level" value={form.level} onChange={e=>setForm({...form, level: e.target.value})} className="border p-3 rounded-xl" /></div>
            <input placeholder="YouTube URL" value={form.videoUrl} onChange={e=>setForm({...form, videoUrl: e.target.value})} className="border p-3 rounded-xl w-full" />
            <input placeholder="Google Map URL" value={form.mapUrl} onChange={e=>setForm({...form, mapUrl: e.target.value})} className="border p-3 rounded-xl w-full" />
            <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4 bg-blue-50 p-4 rounded-xl border border-blue-100"><div><label className="text-xs font-bold text-gray-500 ml-1">Interest (%)</label><input type="number" value={form.interestRate} onChange={e=>setForm({...form, interestRate: e.target.value})} className="border p-3 rounded-xl w-full" /></div><div><label className="text-xs font-bold text-gray-500 ml-1">Max Financing (%)</label><input type="number" value={form.maxMortgagePercent} onChange={e=>setForm({...form, maxMortgagePercent: e.target.value})} className="border p-3 rounded-xl w-full" /></div><div><label className="text-xs font-bold text-gray-500 ml-1">Plans (Months)</label><input value={form.mortgageMonths} onChange={e=>setForm({...form, mortgageMonths: e.target.value})} className="border p-3 rounded-xl w-full" /></div></div>
        ...
        <div className="mt-12 grid gap-4"><h2 className="text-2xl font-bold text-gray-800">Your Properties</h2>{properties.map(p => (<div key={p._id} className="flex flex-col md:flex-row justify-between items-center border p-4 rounded-xl hover:shadow-md transition bg-gray-50"><div className="flex items-center gap-4 mb-4 md:mb-0 w-full"><img src={p.images[0]} className="w-16 h-16 rounded object-cover bg-gray-200"/><div><span className="font-bold text-lg block">{p.title}</span><span className="text-sm text-gray-500">{parseInt(p.price).toLocaleString()} EGP - {p.city}</span></div></div><div className="flex gap-2 w-full md:w-auto"><button onClick={() => startEdit(p)} className="bg-blue-100 text-blue-600 px-4 py-2 rounded-lg font-bold flex-1">Edit</button><button onClick={() => deleteProperty(p._id)} className="bg-red-100 text-red-600 px-4 py-2 rounded-lg font-bold flex-1">Delete</button></div></div>))}</div>
        <div className="mt-16 pt-8 border-t-2 border-gray-100"><h2 className="text-2xl font-bold mb-6 text-gray-800">Partners</h2><form onSubmit={addPartner} className="flex gap-4 mb-6"><input placeholder="Partner Name" value={partnerForm.name} onChange={e => setPartnerForm({...partnerForm, name: e.target.value})} className="border p-3 rounded-xl flex-1"/><input placeholder="Logo URL" value={partnerForm.logo} onChange={e => setPartnerForm({...partnerForm, logo: e.target.value})} className="border p-3 rounded-xl flex-1"/><button className="bg-purple-600 text-white px-6 rounded-xl font-bold">Add</button></form><div className="grid grid-cols-2 md:grid-cols-4 gap-4">{partners.map(p => (<div key={p._id} className="border p-4 rounded-xl flex justify-between items-center bg-white"><span className="font-bold">{p.name}</span><button onClick={() => deletePartner(p._id)} className="text-red-500 font-bold">✕</button></div>))}</div></div>
      </div>
    </div>
  );
}